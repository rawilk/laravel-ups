...
it('can make api requests', function () {
    ...
    expect($response->shipment_identification_number)->not()->toBeEmpty()
        ->and($response->shipment_digest)->not()->toBeEmpty();
});

it('requires a shipment', function () {
    (new ShipConfirm)->getDigest();
})->expectException(BadRequest::class);

it('requires a shipper', function () {
    $shipment = new Shipment;

    (new ShipConfirm)->withShipment($shipment)->getDigest();
})->expectException(BadRequest::class);

it('requires a shipper name', function () {
    $shipment = new Shipment([
        'shipper' => new Shipper,
    ]);
...
it('requires a shipper number', function () {
    $shipment = new Shipment([
        'shipper' => new Shipper([
            'name' => 'foo',
        ]),
...
it('requires a shipper address', function () {
    $shipment = new Shipment([
        'shipper' => new Shipper([
            'name' => 'foo',
